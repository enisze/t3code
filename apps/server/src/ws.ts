        ...
        Effect.gen(function* () {
          ...
          const bootstrapProgram = Effect.gen(function* () {
            if (bootstrap?.createThread) {
              ...
              // as a no-op and continue with the turn against the existing thread
              // instead of failing the whole send.
              const createdEvent = yield* dispatchFromClient({
                type: "thread.create",
              ...
              if (Option.isSome(createdEvent)) {
                ...
                yield* threadDeletionReactor.drainThrough(createdEvent.value.sequence);
                // Stays false for a thread we did not create, so cleanup never
                // deletes it.
                createdThread = true;
              }
            }
