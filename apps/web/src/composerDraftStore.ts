...
// Non-image files (pdf/docx/xlsx/…) staged in the composer. Kept separate from
// images because none of the image machinery (thumbnails, expand preview,
// stash compression) applies; documents only carry the raw File through to the
// send path. In-memory only — not persisted across reloads.
...
interface ComposerDraftStoreState {
  ...
  addDocuments: (threadRef: ComposerThreadTarget, documents: ComposerDocumentAttachment[]) => void;
  removeDocument: (threadRef: ComposerThreadTarget, documentId: string) => void;
  ...
  persist(
    (setBase, get) => {
      ...
      return {
        ...
        addDocuments: (threadRef, documents) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0 || documents.length === 0) {
            return;
          }
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const existingIds = new Set(existing.documents.map((document) => document.id));
            const existingKeys = new Set(
              existing.documents.map((document) => `${document.name}:${document.sizeBytes}`),
            );
            const dedupedIncoming: ComposerDocumentAttachment[] = [];
            for (const document of documents) {
              const dedupKey = `${document.name}:${document.sizeBytes}`;
              if (existingIds.has(document.id) || existingKeys.has(dedupKey)) {
                continue;
              }
              dedupedIncoming.push(document);
        ...
        removeDocument: (threadRef, documentId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) {
            return;
          }
          set((state) => {
            ...
            if (nextDocuments.length === current.documents.length) {
              return state;
            }
            const nextDraft: ComposerThreadDraftState = {
              ...current,
