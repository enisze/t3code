...
export function RightPanelResizeHandle({ handlers, className }: Props) {
  return (
    <div
      ...
      className={cn(
        "group absolute inset-y-0 -left-1.5 z-30 w-3 cursor-col-resize touch-none select-none",
        className,
      )}
      {...handlers}
    >
      <span
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-border/40 transition-colors duration-150 group-hover:bg-border group-active:bg-primary/60"
      />
    </div>
  );
}
